...
async function distillWithResponsesApi(raw: RawCaptureEvent, runtime: ModelProviderRuntime): Promise<DistilledShortTerm> {
  ...
  const prompt = [
    ...
    '- if episode is returned, event_type must be exactly captured and object_kind must be exactly episode',
    '- if episode is returned, polarity must be exactly supporting, conflicting, or insufficient',
    '- if episode is returned, quality.status must be exactly pass, needs_review, or rejected',
              ...
              anyOf: [
                {
                  ...
                  additionalProperties: false,
                  properties: {
                    event_type: { type: 'string', enum: ['captured'] },
                    object_kind: { type: 'string', enum: ['episode'] },
                    signal_type: { type: 'string' },
                    polarity: { type: 'string', enum: ['supporting', 'conflicting', 'insufficient'] },
                    summary: { type: 'string' },
                    ...
                    quality: {
                      ...
                      properties: {
                        ...
                        status: { type: 'string', enum: ['pass', 'needs_review', 'rejected'] },
